...
const first = (inputArray, cbFunction) => {
  return cbFunction(inputArray[0])
}
...
const last = (inputArray, cbFunction) => {
  return cbFunction(inputArray[inputArray.length-1])
}
...
const multiply = (numA, numB, cbFunction) => {
  return cbFunction(numA*numB)
}
...
const contains = (inputArray, name, cbFunction) => {
...
const startWithLetterA = (array) => {
  return array.filter((element) => {
    ...
    if(element[0] === "a" || element[0] === "A"){
      ...
      //console.log(element + " does not start with a")
      return false
    }
  })
}
...
const findProgrammer = (employeeArray) => {
  //filter through the objects looking for a programmer
  let filteredEmployees = employeeArray.filter((person) => {
